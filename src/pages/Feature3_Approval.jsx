...
function Feature3_Approval() {
  ...
  // Mock user_id của GĐ POS (Thường lấy từ localStorage sau khi login)
  const currentPosId = 1; 
  ...
  const pendingList = properties.filter(p => 
    (p.level1_status === 'Chờ POS duyệt' || p.level1_status === 'Chờ duyệt đảm bảo') && p.pos_id === currentPosId
  );

  const historyList = properties.filter(p => 
    (p.level1_status === 'Được duyệt' || p.level1_status === 'Được đảm bảo' || p.level1_status === 'Bị từ chối') && p.pos_id === currentPosId
  );

  const handleApprove = async (type) => {
    if (!selectedProp) return;
    
    const confirmMsg = type === 'Standard' 
      ? `Xác nhận phê duyệt tài sản ${selectedProp.id} vào Kho Chuẩn?`
      : `Xác nhận phê duyệt tài sản ${selectedProp.id} vào Kho Đảm bảo?`;
          ...
          {selectedProp ? (
            <div className="card border-0 shadow-sm h-100 overflow-hidden">
              ...
              <div className="position-relative bg-dark" style={{ height: '300px' }}>
                <img src={`https://images.unsplash.com/photo-1568605114967-8130f3a36994?auto=format&fit=crop&w=1000&q=80`} 
              ...
              <div className="card-body p-4">
                <div className="row g-4">
                  ...
                  <div className="col-md-8">
                    <div className="d-flex justify-content-between align-items-center mb-4 bg-light p-3 rounded-3">
                      ...
                      <div className="text-center px-3">
                        <div className="text-muted small">Hướng</div>
                        <div className="fw-bold fs-5">Đông Nam</div>
                      </div>
                    </div>
                    ...
                    <div className="row mb-4">
                      <div className="col-6 mb-2">
                        <span className="text-muted small">Chiều ngang:</span> <span className="fw-semibold ms-2">4.5 m</span>
                      </div>
                      <div className="col-6 mb-2">
                        <span className="text-muted small">Chiều dài:</span> <span className="fw-semibold ms-2">18 m</span>
                      </div>
                      <div className="col-6 mb-2">
                        <span className="text-muted small">Pháp lý:</span> <span className="fw-semibold ms-2 text-success"><i className="bi bi-shield-check me-1"></i>Sổ hồng riêng</span>
                      </div>
                      <div className="col-6 mb-2">
                        <span className="text-muted small">Đường trước nhà:</span> <span className="fw-semibold ms-2">6 m (Ô tô tránh)</span>
                      </div>
                    </div>
                  ...
                  <div className="col-md-4">
                    <div className="card border-0 bg-primary bg-opacity-10 p-3 mb-4 rounded-4">
                      ...
                      <div className="h3 fw-bold text-primary mb-0">{selectedProp.price?.toLocaleString()} VNĐ</div>
                      <div className="small text-muted">~ {(selectedProp.price / selectedProp.area / 1000000).toFixed(1)} triệu/m²</div>
                    </div>
                ...
                {activeTab === 'pending' ? (
                  <div className="mt-4 border-top pt-4 d-flex gap-3 justify-content-center">
                    {selectedProp.level1_status === 'Chờ POS duyệt' && (
                      <button className="btn btn-success fw-bold px-5 py-2 shadow-sm rounded-pill" onClick={() => handleApprove('Standard')}>
                        <i className="bi bi-check-circle-fill me-2"></i> PHÊ DUYỆT VÀO KHO CHUẨN
                      </button>
                    )}
                    {selectedProp.level1_status === 'Chờ duyệt đảm bảo' && (
                      <button className="btn btn-warning fw-bold px-5 py-2 shadow-sm rounded-pill" onClick={() => handleApprove('Guaranteed')}>
                        <i className="bi bi-shield-fill-check me-2"></i> PHÊ DUYỆT ĐẢM BẢO
                      </button>
                    )}
                    <button className="btn btn-outline-danger fw-bold px-4 py-2 rounded-pill" onClick={() => setShowRejectModal(true)}>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-danger text-white">
                <h5 className="modal-title fw-bold">Từ chối hồ sơ {selectedProp?.id}</h5>
